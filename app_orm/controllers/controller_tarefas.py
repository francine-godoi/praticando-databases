...
class ControllerTarefas:
    ...
    def adicionar_tarefa(self) -> None:
        ...
        tarefa = Tarefa(self.id_usuario, descricao, importancia, datetime.today().strftime("%d/%m/%Y"))
        try:
            session.add(tarefa)
            session.commit()        
    ...
    def existe_tarefas_andamento(self) -> bool:
        tarefas = session.query(Tarefa).filter(Tarefa.id_usuario==self.id_usuario).where(Tarefa.status=="A").all()
        if not tarefas:
    ...
    def selecionar_tarefa_por_id(self) -> int | Tarefa:
        ...
        tarefa_selecionada = session.query(Tarefa).filter(Tarefa.id_tarefa==id_tarefa).where(Tarefa.status=="A").where(Tarefa.id_usuario==self.id_usuario).first()
    ...
    def listar_todas_tarefas(self) -> None:
        tarefas = session.query(Tarefa).filter(Tarefa.id_usuario==self.id_usuario).all()
        if not tarefas:
    ...
    def listar_tarefas_por_status(self, tipo_status: str) -> None:
        tarefas = session.query(Tarefa).filter(Tarefa.id_usuario==self.id_usuario).where(Tarefa.status==tipo_status).all()
        if not tarefas:
            print("Nenhuma tarefa encontrada.\n")
            return
        self.tela_tarefas.exibir_tarefas(tarefas)
    ...
    def listar_tarefas_por_importancia(self) -> None:        
        ...
        tarefas = session.query(Tarefa).filter(Tarefa.id_usuario==self.id_usuario).where(Tarefa.importancia==importancia).all()
        if not tarefas:
            print("Nenhuma tarefa encontrada.\n")
            return
        self.tela_tarefas.exibir_tarefas(tarefas)
    ...
    def listar_tarefas_por_data(self, tipo_data: str) -> None:
        ...
        if tipo_data == "criado_em":
            tarefas = session.query(Tarefa).filter(Tarefa.id_usuario==self.id_usuario).where(Tarefa.criado_em>=data_inicial).where(Tarefa.criado_em<=data_final)
        elif tipo_data == "finalizado_em":
            tarefas = session.query(Tarefa).filter(Tarefa.id_usuario==self.id_usuario).where(Tarefa.finalizado_em>=data_inicial).where(Tarefa.finalizado_em<=data_final)
